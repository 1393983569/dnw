<template>
  <div>
    <Card style="min-height: 100px; margin-bottom: 20px">
      <div>
        <Button type="primary" @click="selectCombo">选择套餐</Button>
      </div>
      <Divider dashed />
...
// getSetMeal 查询套餐详情
import { getPageSetMeal, getListDiscount, addCusSetMeal } from '@/api/cuManagement/InformationEditing/InformationEditing'
...
export default ({
  ...
  data () {
    return {
            ...
            h('Select', {
              ...
              on: {
                'on-change': (e) => {
                  ...
                  this.dataTable[index].finalPrice = Math.floor(((provinceValue * parseInt(this.dataTable[index].marketPrice)) * 100)) / 100
                  this.$emit('onChangeCombo', this.dataTable)
                }
              },
              // 清空
              'on-clear': () => {
                this.dataTable[index].disId = ''
                this.dataTable[index].finalPrice = this.dataTable[index].marketPrice
                this.$emit('onChangeCombo', this.dataTable)
              }
            },
      ...
      selectData: [{key: 'mealName', name: '名称'}],
      // 分页
      pageTotal: 1,
      // 页码
      pageNum: 1,
  ...
  methods: {
    ...
    selectCombo () {
      this.modal = true
      getPageSetMeal(this.pageNum, this.selectValue).then(res => {
        this.dataSelect = []
        this.pageTotal = res.info.pageTotal
        res.info.data.map(item => {
          this.dataSelect.push({
            catName: item.mealName,
            marketPrice: item.mealPrice,
            // 打折后价格
            finalPrice: item.mealPrice,
            // 产品id
            setmealId: item.id,
            // 折扣id
            disId: '',
            accountsPayable: item.mealPrice,
            referee: ''
          })
        })
      }).catch(err => {
